    ...
    <div class="md-layout">
        ...
        <md-card>
            <md-card-header class="d-flex justify-content-between" data-background-color="green">
                <h4 class="title h3">{{ $route.path.includes('editar') ? 'Editar' : 'Adicionar' }} forma de pagamento</h4>
            </md-card-header>
            <md-card-content>
                    ...
                    <div class="row">
                        <b-form-group label-for="nome" class="col-4">
                            <md-field :class="getValidationClass('nome')">
                                <label>Nome</label>
                                <md-input id="nome" type="text" v-model="form.formaPagamento.nome" autocomplete="off"
                                    :disabled="carregando">
                                </md-input>
                                <span class="md-error" v-if="!$v.form.formaPagamento.nome.required">Descrição
                                    obrigatória!</span>
                            </md-field>
                        </b-form-group>
                        ...
                        <b-form-group label-for="tipo" class="col-4">
                            <md-field :class="getValidationClass('tipo')">
                                <label>Tipo</label>
                                <md-select id="tipo" v-model="form.formaPagamento.tipo" autocomplete="off"
                                :disabled="carregando">
                                ...
                                <span class="md-error" v-if="!$v.form.formaPagamento.tipo.required">Descrição
                                    obrigatória!</span>
                            </md-field>
                        </b-form-group>

                        <b-form-group v-if="cartaoAtivo" label-for="tipo_cartao" class="col-4">
                            <md-field :class="getValidationClass('tipo')">
                                <label>Tipo de cartão</label>
                                <md-select id="tipo_cartao" v-model="form.formaPagamento.tipo_cartao" autocomplete="off"
                                :disabled="carregando">
...
export default {
    ...
    methods: {
        async carregaDados() {
            ...
            await this.$api.get(`/formas_pagamento/${action}`)
                .then(response => {
                    this.form.formaPagamento = response.data.FormaPagamento ?? this.form.formaPagamento;
        ...
        async salvaDados() {
            ...
            if (this.$route.path.includes('editar')) {
                ...
                response = await this.$api.put(`/formas_pagamento/editar/${id}`, this.form.formaPagamento)
            } else {
                response = await this.$api.post('/formas_pagamento/adicionar', this.form.formaPagamento)
            }
