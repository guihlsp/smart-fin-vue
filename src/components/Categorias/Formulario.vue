    ...
    <div class="md-layout">
        ...
        <md-card>
            <md-card-header class="d-flex justify-content-between" data-background-color="green">
                <h4 class="title h3">{{ $route.path.includes('editar') ? 'Editar' : 'Adicionar' }} categoria</h4>
            </md-card-header>
            <md-card-content>
                <b-form @submit.prevent="valida" novalidate class="row">
                    <b-form-group label-for="descricao" class="col-6">
                        <md-field :class="getValidationClass('descricao')">
                            ...
                            <md-input id="descricao" type="text" v-model="form.categoria.descricao"
                                autocomplete="off" :disabled="carregando">
                            </md-input>
                            <span class="md-error" v-if="!$v.form.categoria.descricao.required">Descrição obrigatória!</span>
                        </md-field>
                    </b-form-group>
                    ...
                    <b-form-group label-for="ativa" class="col-6 md-layout-item">
                        <md-field>
                            <label>Ativa</label>
                            <md-select id="ativa" v-model="form.categoria.ativa"
                                autocomplete="off" :disabled="carregando">
                    ...
                    <div class="mt-3">
                        ...
                        <md-button style="margin-left: 15px" to="/categorias">
                            Cancelar
                        </md-button>
                    </div>
                </b-form>
...
export default {
    ...
    methods: {
        async carregaDados() {
            ...
            await this.$api.get(`/categorias/editar/${id}`)
            .then(response => {
        ...
        async salvaDados() {
            ...
            if (this.$route.path.includes('editar')) {
                ...
                response = await this.$api.put(`/categorias/editar/${id}`, this.form.categoria)
            } else {
                response = await this.$api.post('/categorias/adicionar', this.form.categoria)
            }
