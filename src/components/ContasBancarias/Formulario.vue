                ...
                <b-form @submit.prevent="valida" novalidate class="row">
                    <b-form-group label-for="descricao" class="col-6">
                        <md-field :class="getValidationClass('descricao')">
                            ...
                            <md-input id="descricao" type="text" v-model="form.contaBancaria.descricao"
                                autocomplete="off" :disabled="carregando">
                            </md-input>
                            <span class="md-error" v-if="!$v.form.contaBancaria.descricao.required">Descrição obrigatória!</span>
                        </md-field>
                    </b-form-group>
                    ...
                    <b-form-group label-for="ativa" class="col-6 md-layout-item">
                        <md-field>
                            <label>Ativa</label>
                            <md-select id="ativa" v-model="form.contaBancaria.ativa"
                                autocomplete="off" :disabled="carregando">
...
export default {
    ...
    methods: {
        async carregaDados() {
            ...
            await this.$api.get(`/contas_bancarias/editar/${id}`)
            .then(response => {
        ...
        async salvaDados() {
            ...
            if (this.$route.path.includes('editar')) {
                ...
                response = await this.$api.put(`/contas_bancarias/editar/${id}`, this.form.contaBancaria)
            } else {
                response = await this.$api.post('/contas_bancarias/adicionar', this.form.contaBancaria)
            }
